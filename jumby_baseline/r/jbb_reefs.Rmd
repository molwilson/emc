...
rf_recruits_tot <- rf_recruits %>%
  ...
  summarize(density = mean(count)) %>% # need to check actual size of quadrat 
  group_by(site, size) %>%
...
#### Density of *Diadema* by site and phase
* Need to convert to density when I check belt width with GRB
```{r}
diadema <- rf_recruits <- read_excel(here("jumby_baseline", "data", "JBB_reefs.xlsx"), sheet = "inverts") %>%  
  ...
  pivot_longer(cols = starts_with("diadema"),
               ...
               values_to = "count") %>%
  group_by(site, phase) %>%
  summarize(density_mean = mean(count/20),
            density_se = sd(count/20)/sqrt(n())) # 2x10m belt?

ggplot(diadema, aes(x = phase, y = density_mean)) +
