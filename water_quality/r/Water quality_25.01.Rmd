...
wq_data <- ysi %>%
  ...
  mutate(n_colonies = replace_na(as.numeric(str_replace(n_colonies, ">" ,"")), 0),
         ...
         site_cat = if_else(site_code %in% c("YIE", "GIE", "YIC", "GIC"), "Control", "Treatment")) %>%
...
data_mon <- wq_data %>%
  filter(site_code %in% c("NBRB", "NBRM", "EC", "NBA", "LBI", "GIA", "ML", "RBA", "RRS", "GOE", "TPBN", "GIE", "GIC", "YIE", "YIC", "DB", "MRCH", "COV", "GEB", "FHB", "YIN", "YRS", "BDB", "LDB", "MRYC", "EBB", "EBBW") & depth_m == 1) %>% # GEB and COV currently combined as Exchange Bay - Club Hotel
  mutate(site_name = factor(site_name, levels = c("Emerald Cove", 
                                                  ...
                                                  "Ricketts Restoration Site",
                                                  "Garden of Eden", 
                                                  "Ten Pound Bay Nursery", 
                                                  "Little Deep Bay", 
                                                  "Great Deep Bay", 
                                                  "York Island Nursery", 
                                                  "York Restoration Site",
                                                  "Friars Head Bay", 
                                                  "Exchange Bay - Turtle Beach", 
                                                  "Exchange Bay - MRC Hotel",
                                                  "Exchange Bay - Boom E",
                                                  "Exchange Bay - Boom W",
                                                  "Green Island Offshore Control", 
                                                  "Green Island Inshore Control",
                                                  "York Island Offshore Control",
                                                  "York Island Inshore Control"))) %>%
  mutate(site_name = fct_rev(site_name)) %>% # flipped axis plot needs reversed order
  filter(as.Date(date) < "2024-12-31") # for 2024 report
...
ggsave(here("water_quality", "figs", "2024", "ph.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(turb_point, turb_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024", "turbidity.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(do_point, do_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024", "do.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(temp_point, temp_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024", "temp.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(chlor_point, chlor_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024", "chlor.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(phyco_point, phyco_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024", "phyco.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(entero_point, entero_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024", "entero.png"),  width = plot_dim_w, height = plot_dim_h)
```
...
ggplot() +
  ...
  facet_wrap(. ~ factor(date_label, levels = c("Oct 2023", "Nov 2023", "Dec 2023", "Jan 2024", "Feb 2024", "Mar 2024", "Apr 2024", "May 2024", "Jun 2024", "Jul 2024", "Aug 2024", "Sep 2024", "Oct 2024", "Nov 2024", "Dec 2024"))) +
...
ggsave(here("water_quality", "figs", "2024", "entero_map.png"), width = 10, height = 9)
```

```{r}
# map for MRC talk slides

ggplot() +
  geom_sf(data = atg, fill = "slategray", color = "slategray") + # ATG basemap
  coord_sf(xlim = lons, ylim = lats, expand = FALSE) + # setting map boundaries
  geom_point(data_mon %>% filter(date_label %in% c("Dec 2023", "Jan 2024", "Feb 2024", "Mar 2024", "Apr 2024", "May 2024", "Jun 2024", "Jul 2024", "Aug 2024", "Sep 2024", "Oct 2024", "Nov 2024")), 
             mapping = aes(x = longitude, y = latitude, color = entero_yn, size = n_colonies), 
             alpha = 0.7) + # add sites
  scale_color_manual(values = c("turquoise", "tomato")) +
  facet_wrap(. ~ factor(date_label, levels = c("Dec 2023", "Jan 2024", "Feb 2024", "Mar 2024", "Apr 2024", "May 2024", "Jun 2024", "Jul 2024", "Aug 2024", "Sep 2024", "Oct 2024", "Nov 2024")),
             nrow = 2) +
  labs(size = expression("Concentration (CFU "*mL^-1*")"),
       color = expression("Enterococci detection")) +
  theme_bw() +
...
ggsave(here("water_quality", "figs", "2024", "entero_map_mrc.png"), width = 10, height = 9)
```
...
ggplot(data_mon %>% filter(site_code %in% c("GIA", "RBA", "NBA", "GOE")), 
  ...
  theme_bw()
ggsave(here("water_quality", "figs", "2024", "entero_boats.png"), width = 6, height = 4)
```
...
ggarrange(entero_phyco, entero_chlor, 
          ncol = 2, nrow = 1)
ggsave(here("water_quality", "figs", "2024", "entero_chlor.png"), width = 6, height = 3)
```
...
ggplot() +
  ...
  scale_x_datetime(date_breaks = "2 months",
                   date_labels = "%b %Y") +
  theme_bw()
ggsave(here("water_quality", "figs", "2024", "precip.png"), width = 6, height = 4)
...
ggsave(here("water_quality", "figs", "2024", "precip_prior.png"), width = 6, height = 4)
...
ggplot() +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave(here("water_quality", "figs", "2024", "sargassum.png"), width = 6, height = 4)
```
...
ggsave(here("water_quality", "figs", "2024", "sarg.png"),  width = 8, height = 8)
