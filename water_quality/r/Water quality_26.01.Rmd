...
ysi_raw <- read_excel(here("water_quality", "data_raw", "Water quality data.xlsx"), sheet = "ysi complete") %>% 
  rename_with(tolower) %>% # prevents clean_names() from adding extra underscores before caps
  clean_names() %>%
  rename(site_code_depth = site_name)

ysi_omissions <- read_excel(here("water_quality", "data_raw", "Water quality data.xlsx"), sheet = "ysi report omissions") %>%
  clean_names()

# need to clean this with new var names
# clean site names - caps, corington
ysi <- ysi_raw %>%
  anti_join(
    ysi_omissions,
...
ysi_dup <- ysi %>% group_by(site_code_depth, date) %>%
  summarize(count = n()) %>%
