...
- Some duplicates have been removed directly from the dataset and some via code - need to make that consistent
...
ysi <- read_excel(here("water_quality", "data_raw", "Water quality data.xlsx"), sheet = "ysi") %>% 
  ...
  filter(as.character(date) != "2023-10-05" # removing incomplete/aborted day 
          & as.character(time) %nin%	c("1899-12-31 11:38:13", 	"1899-12-31 14:45:28", "1899-12-31 09:41:43", "1899-12-31 09:29:43", "1899-12-31 10:36:28", "1899-12-31 12:55:39", "1899-12-31 14:20:21", "1899-12-31 11:25:50", "1899-12-31 12:02:20", "1899-12-31 12:12:54")) # removing duplicate samples - some have been removed from dataset directly, eventually all should be
...
sargassum_regional <- read_excel(here("water_quality", "data_raw", "Water quality data.xlsx"), sheet = "sargassum_regional") %>%
  ...
  mutate(date = as.Date(date, origin = "1899-12-30")) # converting from weird excel format
...
wq_data <- ysi %>%
  select(date, site_code_depth, temperature_c = temperature_c_24, ph, do_mg_l, sal_psu, phycoerythrin_rfu, chlorophyll_rfu) %>% # do_percent_sat, conductivity - taking these out for now because missing for Cades Jan 2025 and its throwing off the dataset
  drop_na() %>%
  left_join(ysi %>% # joining data from 5th probe
  ...
  mutate(longitude = as.numeric(longitude),
         ...
         #n_colonies = replace_na(n_colonies, 0),
         ...
         site_cat = if_else(site_code %in% c("YIE", "GIE", "YIC", "GIC", "CRS"), "Control", "Treatment")) %>%
  select(date, doy, month, year, date_label, date_cat, site_code_depth, site_code, depth_m, site_name, site_cat, latitude, longitude, n_boats, leachate, temperature_c, ph, do_mg_l, sal_psu, phycoerythrin_rfu, chlorophyll_rfu, turbidity_fnu, n_colonies, entero_yn) %>% # do_percent_sat, conductivity_us_cm, 
  distinct()

# streamlining data for graphs to include only regularly monitored sites at 1m depth
data_mon <- wq_data %>%
  mutate(depth_m = if_else(site_code_depth == "CRS3" & date == as.Date("2025-01-20"), 1, depth_m)) %>% # missing 1m depth for CRS in Jan 2025, so switching 3m to use as 1m instead
  filter(site_code %in% c("NBRB", "NBRM", "EC", "NBA", "GIA", "ML", "RBA", "GOE", "TPBN", "GIE", "GIC", "YIE", "YIC", "DB", "MRCH", "COV", "GEB", "FHB", "YIN", "BDB", "LDB", "MRYC", "EBB", "EBBW", "CN", "CRS") & depth_m == 1) %>% # GEB and COV currently combined as Exchange Bay - Club Hotel, GIC/GIE and YIC/YIE also combined
  mutate(site_name = factor(site_name, levels = c("Emerald Cove", 
                                                  ...
                                                  "Exchange Bay - Boom E",
                                                  "Exchange Bay - Boom W",
                                                  "Green Island Control", 
                                                  "York Island Control",
                                                  "Cades Nursery",
                                                  "Cades Restoration Site"
                                                          ))) %>%
  mutate(site_name = fct_rev(site_name)) %>% # flipped axis plot needs reversed order
  filter(as.Date(date) < "2025-06-30") # for 2025 report
...
ggsave(here("water_quality", "figs", "2024-2025", "sites_map.png"), width = 5, height = 4)
...
ggsave(here("water_quality", "figs", "2024-2025", "ph.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(turb_point, turb_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024-2025", "turbidity.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(do_point, do_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024-2025", "do.png"),  width = plot_dim_w, height = plot_dim_h)

# temp
data_mon_temp <- data_mon %>% filter(site_code != "GEB")
temp_point <- flpoint_site_ref(ref_temp, data_mon_temp, temperature_c, lab_temp, ylims_temp) +
  geom_hline(yintercept = ref_temp_bleaching, color = "salmon", linetype = "dashed")
temp_line <- line_time_ref(ref_temp, lab_ref_temp, data_mon_temp, temperature_c, lab_temp, ylims_temp) +
...
ggarrange(temp_point, temp_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024-2025", "temp.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(chlor_point, chlor_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024-2025", "chlor.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(phyco_point, phyco_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024-2025", "phyco.png"),  width = plot_dim_w, height = plot_dim_h)
...
ggarrange(entero_point, entero_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2024-2025", "entero.png"),  width = plot_dim_w, height = plot_dim_h)
```
...
ggplot(data_mon %>% 
  ...
  theme_minimal()
ggsave(here("water_quality", "figs", "2024-2025", "entero_interannual.png"), width = 6, height = 4)
```
...
ggsave(here("water_quality", "figs", "2024-2025", "entero_map.png"), width = 10, height = 9)
```
...
ggplot(data_mon %>% filter(site_code %in% c("GIA", "RBA", "NBA", "GOE")), 
  ...
  theme_bw()
ggsave(here("water_quality", "figs", "2024-2025", "entero_boats.png"), width = 6, height = 4)
```
...
entero_phyco <- ggplot(data_mon,
       aes(x = phycoerythrin_rfu, y = n_colonies)) +
  geom_point(alpha = 0.6, size = 2) +
  sm_statCorr() +
...
ggarrange(entero_phyco, entero_chlor, 
          ncol = 2, nrow = 1)
ggsave(here("water_quality", "figs", "2024-2025", "entero_chlor.png"), width = 6, height = 3)
```
...
ggsave(here("water_quality", "figs", "2024-2025", "precip.png"), width = 6, height = 4)
...
ggsave(here("water_quality", "figs", "2024-2025", "precip_prior.png"), width = 6, height = 4)
...
ggplot() +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave(here("water_quality", "figs", "2024-2025", "sargassum.png"), width = 6, height = 4)
```
...
ggsave(here("water_quality", "figs", "2024-2025", "sarg.png"),  width = 8, height = 8)
