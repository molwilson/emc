...
# make controls dark blue
# incoporate new control sites
# stop at Dec 2024
# take mean when more than one sample per site/date are taken
# take GEB out of this analysis...?
...
wq_data <- ysi %>%
  ...
  mutate(n_colonies = replace_na(as.numeric(str_replace(n_colonies, ">" ,"")), 0),
  ...
  select(date, date_label, date_cat, site_code_depth, site_code, depth_m, site_name, site_cat, latitude, longitude, n_boats, leachate, temperature_c, ph, do_mg_l, do_percent_sat, conductivity_us_cm, sal_psu, phycoerythrin_rfu, chlorophyll_rfu, turbidity_fnu, n_colonies, entero_yn) %>%
  distinct()

# streamlining data for graphs to include only regularly monitored sites at 1m depth
data_mon <- wq_data %>%
  filter(site_code %in% c("NBRB", "NBRM", "EC", "NBA", "GIA", "ML", "RBA", "GOE", "TPBN", "GIE", "GIC", "YIE", "YIC", "DB", "MRCH", "COV", "GEB", "FHB", "YIN", "BDB", "LDB", "MRYC", "EBB", "EBBW", "CN", "CRS") & depth_m == 1) %>% # GEB and COV currently combined as Exchange Bay - Club Hotel
...
ggsave(here("water_quality", "figs", "2024-2025", "entero_map_nonsuch.png"), width = 10, height = 9)
```
