...
# accounting for transition in control sites (GIE -> GIC, YIE -> YIC) to remove overlap
first_gic <- data_mon %>%
  filter(site_code == "GIC") %>%
  summarise(first_gic = min(date)) %>%
  pull(first_gic)
...
ggplot() +
  ...
  geom_point(data_mon %>% 
             ...
             alpha = 0.9, color = "orange", size = 2) + # add sites
...
data_mon_entero <- data_mon %>%
  select(year, date_label, site_name, latitude, longitude, n_colonies, entero_yn)

aug_2025 <- data_mon_entero %>%
...
data_mon_entero <- bind_rows(data_mon_entero, aug_2025)
...
ggplot() +
  geom_sf(data = atg, fill = "slategray", color = "slategray") + # ATG basemap
  coord_sf(xlim = lons, ylim = lats, expand = FALSE) + # setting map boundaries
  geom_point(data_mon_entero %>% filter(year %in% c(2024, 2025)), 
             mapping = aes(x = longitude, y = latitude, color = entero_yn, size = n_colonies), 
             alpha = 0.7) + # add sites
  scale_color_manual(values = c("turquoise", "tomato")) +
  facet_wrap(. ~ factor(date_label, levels = c("Oct 2023", "Nov 2023", "Dec 2023", "Jan 2024", "Feb 2024", "Mar 2024", "Apr 2024", "May 2024", "Jun 2024", "Jul 2024", "Aug 2024", "Sep 2024", "Oct 2024", "Nov 2024", "Dec 2024", "Jan 2025", "Feb 2025", "Mar 2025", "Apr 2025", "May 2025", "Jun 2025", "Jul 2025", "Aug 2025", "Sep 2025", "Oct 2025", "Nov 2025", "Dec 2025", "Jan 2026", "Feb 2026", "Mar 2026", "Apr 2026", "May 2026")),
             ncol = 12) +
...
ggsave(here(fig_folder, "entero_map.png"), width = 10, height = 6)
```
