...
ysi <- read_excel(here("water_quality", "data_raw", "Water quality data.xlsx"), sheet = "ysi") %>% 
  ...
  filter(as.character(date) != "2023-10-05" # removing incomplete/aborted day 
          & as.character(time) != 	c("1899-12-31 11:38:13", 	"1899-12-31 14:45:28", "1899-12-31 09:41:43", "1899-12-31 09:29:43")) # removing duplicate samples
...
lab_ref_temp_bleaching <- "Bleaching threshold (NOAA)"
ylims_temp <- c(22, (max(data_mon$temperature_c, na.rm = T) + 1))

# enterococcus
lab_entero <- expression(paste(italic("Enterococcus" ), " spp. (colonies "*mL^-1*")"))
ref_entero = data.frame(xmin = -Inf, xmax = Inf, ymin = 0, ymax = 7/100) # below 7/100mL (EPA)
...
# chlorophyll a
lab_chlor <- expression(paste("Chlorophyll ", italic("a")))
...
- How do we want to label control (offshore) vs. treatment sites?
- Should we be calling "Exchange Bay - Dansby" -> "Exchange Bay - Turtle Beach" for political reasons?

#### To-dos:

- Messing around with how I can get control/offshore lines to both plot in top layer to make line plots more clear
- Need to figure out how to label bleaching threshold in temperature graph without changing other line aesthetics
...
ggsave(here("water_quality", "figs", "2023-2024", "ph.png"),  width = 10, height = 6)
...
ggarrange(turb_point, turb_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2023-2024", "turbidity.png"),  width = 10, height = 6)
...
ggarrange(do_point, do_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2023-2024", "do.png"),  width = 10, height = 6)
...
temp_line <- line_time_ref(ref_temp, lab_ref_temp, data_mon, temperature_c, lab_temp, ylims_temp) +
  geom_hline(yintercept = ref_temp_bleaching, color = "salmon", linetype = "dashed", linetype = "test") +
  scale_linetype_manual(name = "")

ggarrange(temp_point, temp_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2023-2024", "temp.png"),  width = 10, height = 6)
...
ggarrange(chlor_point, chlor_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2023-2024", "chlor.png"),  width = 10, height = 6)
...
ggarrange(phyco_point, phyco_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2023-2024", "phyco.png"),  width = 10, height = 6)
...
ggarrange(entero_point, entero_line, 
          ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")
ggsave(here("water_quality", "figs", "2023-2024", "entero.png"),  width = 10, height = 6)
```
...
ggplot(data_mon %>% filter(site_code %in% c("GIA", "RBA", "NBA", "GOE")), 
  ...
  labs(x = "Number of boats in anchorages", y = expression(paste(italic("Enterococcus" ), " spp. (colonies "*mL^-1*")"))) +
  theme_bw() +
...
sarg <- function(data_ref, lab_ref, y, ylab, ylims) {
  ggplot() +
    geom_rect(data = data_ref, 
              aes(xmin = as.POSIXct(-Inf), xmax = as.POSIXct(Inf), ymin = ymin, ymax = ymax, fill = lab_ref),
              alpha = a_range) +
    ...
    geom_point(data = wq_data %>% 
                 ...
                 mutate(site_cat = if_else(site_cat == "Control", "Offshore control", "Seaweed boom")),
       aes(x = date, y = {{y}}, color = site_cat, shape = as.character(depth_m)),
       alpha = 0.8, size = 2) +
    scale_color_manual(values = c("cadetblue", "orange")) +
    ylim(ylims) + 
    labs(x = "", y = ylab, shape = "Depth (m)", color = "Sampling location", fill = "") +
    theme_bw()
}

sarg(ref_ph, lab_ref_ph, ph, lab_ph, ylims_ph)
sarg(ref_do, lab_ref_do, do_mg_l, lab_do, ylims_do)
sarg(ref_turb, lab_ref_turb, turbidity_fnu, lab_turb, ylims_turb)
# sarg(ref_entero, lab_ref_entero, n_colonies, lab_entero, ylims_entero)
