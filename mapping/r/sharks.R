...
# convert to sf
shark_sf <- st_as_sf(shark_data, coords = c("lon", "lat"), crs = 4326)
...
xlim_3857 <- c(bb["xmin"], bb["xmax"])
ylim_3857 <- c(bb["ymin"], bb["ymax"])

# still map

## buffered
ggplot() + 
  annotation_map_tile(type = esri_ocean, progress = "none", zoomin = 1) +
...
p_anim <- ggplot() +
  annotation_map_tile(type = esri_world_imagery, progress = "none", zoomin = 2) +
  ...
  geom_point(data = shark_df,
             ...
             size = 2) +
  scale_color_manual(values = c("coral2", "gold")) +
  coord_sf(xlim = c(-62.15, -61.6), ylim = c(16.9, 17.3), expand = FALSE) +
...
anim <- animate(
  ...
  renderer = gifski_renderer(here::here("mapping", "figs", "sharks_track_esri.gif"))
)
