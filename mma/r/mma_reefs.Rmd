...
* Need to fix species names for other competitors in data sheet
```{r}
...
ggsave(here("mma", "figs", "reef_comp_spp.png"))
...
```{r, eval = F}
# graph
ggplot(rf_fish_fam_site, aes(x = family_com, y = bm_mean)) +
...
ggplot(rf_fish_fam, aes(x = family_com, y = bm_mean)) +
  ...
  theme_bw()
  # theme(axis.text.x = element_text(angle = 45, h = 1))
ggsave(here("mma", "figs", "reef_fish_fam_bm.png"))
...
ggplot(rf_fish_fam_site %>%
  ...
  annotate("text", x = 8.5, y = 162, size = 3, hjust = 1, label=c('Eastern Caribbean mean unfished biomass')) +
  geom_hline(yintercept = 119, linetype = "dashed", color = "black") +
  annotate("text", x = 8.5, y = 126, size = 3, hjust = 1, label=c('Antigua mean biomass')) +
  labs(x = "", y = expression(Parrotfish~biomass~(kg~ha^-1))) +
  theme_bw() +
  theme(axis.text.x = element_blank())
ggsave(here("mma", "figs", "reef_scarid_bm.png"), height = 3, width = 5)
```
...
ggplot(rf_fish_fam_site %>%
  ...
  annotate("text", x = 8.5, y = 142, size = 3, hjust = 1, label=c('Antigua mean biomass')) +
...
ggplot(rf_fish_spp, aes(x = reorder(species_name, bm_mean), y = bm_mean)) +
  ...
  geom_errorbar(aes(ymin = bm_mean - bm_se, ymax = bm_mean + bm_se), width = .2,
                 position = position_dodge(.9)) +
  coord_flip() +
  labs(x = "Species", y = "Biomass (kg/ha)") +
  theme_bw() +
  theme(axis.text.y = element_text(face = "italic"))
