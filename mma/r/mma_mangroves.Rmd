...
mg_fish_site <- mg_fish %>% 
  ...
  summarize(biomass_tot = sum(biomass)/1000/30*10000) %>% # kg/ha, 30m^2 transects
  group_by(site) %>%
...
mg_fish_fam_site <- mg_fish %>% 
  ...
  summarize(biomass_tot = sum(biomass)/1000/30*10000, # kg/ha
            length = mean(length)) %>%
...
test3 <- mg_fish %>% 
  ...
  group_by(site, transect, family_c) %>%
  summarize(biomass_tot = sum(biomass), # don't change units for this test
            length = mean(length)) %>%
  filter(site == "Emerald Cove" & family_c == "Sphyraenidae")
test4 <- test3 %>% 
  group_by(site, family_c) %>%
