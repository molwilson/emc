...
```{r}
mgr_invert_cat_site <- mgr_invert %>%
  expand(nesting(site, transect), type) %>%
  left_join(mgr_invert %>%
...
ggplot(mgr_invert_spp, aes(x = reorder(species, abundance_mean), y = abundance_mean)) +
  geom_col(position = "dodge", fill = "seagreen") +
  geom_errorbar(aes(ymin = abundance_mean - abundance_se, ymax = abundance_mean + abundance_se), width = .2,
                 position = position_dodge(.9)) +
  coord_flip() +
...
mgr_fish <- read_excel(here("mma", "data", "mma_mangroves.xlsx"), sheet = "fish") %>%  
  ...
  mutate(biomass = as.numeric(biomass),
         ...
         phase = case_when(phase == "j" ~ "Juvenile",
                           phase == "i" ~ "Initial",
                           phase == "t" ~ "Terminal"),
         family_con = if_else(family %in% c("Atherinidae", "Clupeidae", "Scaridae", "Haemulidae", "Lutjanidae", "Sphyraenidae"), family, "Other")) # reducing number of families
```

## Mean total biomass by site
```{r}
mgr_fish_site <- mgr_fish %>% 
...
ggplot(mgr_fish_site, aes(x = site, y = biomass_mean)) +
...
mgr_fish_family_site <- mgr_fish %>% 
  expand(nesting(site, transect), family_con) %>%
  left_join(mgr_fish %>%
      select(site, transect, family_con, length, biomass)) %>%
  mutate(biomass = if_else(is.na(biomass), 0, biomass),
         length = if_else(is.na(length), 0, length)) %>%
  group_by(site, transect, family_con) %>%
  summarize(biomass_tot = sum(biomass)/1000/120*10000, #kg/ha
            length = mean(length)) %>%
  group_by(site, family_con) %>%
  summarize(biomass_mean = mean(biomass_tot),
            biomass_se = sd(biomass_tot)/sqrt(n()),
...
ggplot(mgr_fish_family %>% 
         filter(biomass_mean >0 & site == "Mean across sites"), 
       aes(x = reorder(family_con, -biomass_mean), y = biomass_mean)) +
...
ggplot(mgr_fish_family %>% 
         filter(biomass_mean >0 & site != "Mean across sites"), 
       aes(x = site, y = biomass_mean, fill = family_con)) +
  geom_col(position = "dodge") +
  geom_errorbar(aes(ymin = biomass_mean - biomass_se, ymax = biomass_mean + biomass_se), width = .2,
                 position = position_dodge(.9)) +
...
mgr_fish_spp <- mgr_fish_spp_site %>%
...
ggplot(mgr_fish_spp, aes(x = reorder(species_name, biomass_mean), y = biomass_mean)) +
...
```{r}
ggplot(mgr_fish, aes(x = site, y = length)) +
  geom_violin() +
  theme_bw()
