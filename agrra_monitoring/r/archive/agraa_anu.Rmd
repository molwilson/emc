...
library(kableExtra) # for knitting tables - do I need this?
...
##### integrating algal canopy heights - do this earlier??
...
benthic_site <- left_join(benthic_cover_site, algal_height_site, by = c("site", "site_cat", "year"))

benthic_year <- benthic_site %>%
  select(!contains("_sd")) %>%
  rename_with(~ str_remove(.x, "_mean")) %>%
  group_by(year, site_cat) %>%
  summarise(across(c(benthic_var),
...
```{r}
benthic_site_long <- benthic_site %>%
  select(year, site_cat, site, contains("mean")) %>%
  pivot_longer(cols = contains("mean"),
               names_to = "variable",
               values_to = "value") %>%
  group_by(site_cat, site, variable, year) %>%
  summarise(value = mean(value, na.rm = TRUE), .groups = "drop")

benthic_year_long <- benthic_site_long %>%
  group_by(year, site_cat, variable) %>%
  summarise(
...
benthic_sig_table <- benthic_site_long %>%
  group_by(site_cat, variable) %>%
  summarise(
    values_by_year = list(split(value, year)),
    .groups = "drop"
  ) %>%
  mutate(
    # paired Wilcoxon test for nonparametric data
    p_value = map_dbl(values_by_year, ~ wilcox.test(.x[[1]], .x[[2]], paired = TRUE)$p.value),
    sig_label = ifelse(p_value < 0.05, "*", "ns")
  ) %>%
  select(site_cat, variable, p_value, sig_label)

### plot
make_lp_summary <- function(summary_df, sig_df, variable_name, ylab) {
  ...
  plot_df <- plot_df %>%
    left_join(sig_df_var, by = c("site_cat", "variable")) %>%
    mutate(
      sig_x = as.numeric(year) + 0.3,      # offset to the right
      sig_y = mean - .001
    )
  
  p <- ggplot(plot_df, aes(x = year, y = mean, group = site_cat, color = site_cat)) +
    geom_point(size = 3) +
    geom_line(size = 1) +
    geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 0.2) +
    geom_text(
      data = plot_df %>% filter(year == "2025"),
      aes(x = sig_x, y = sig_y, label = sig_label, color = site_cat),
      inherit.aes = FALSE,
      size = 5,
      show.legend = FALSE
    ) +
    ...
    labs(x = "", y = ylab, color = "Site Category", group = "Site Category") +
    theme_pub() +
...
p_fma <- make_lp_summary(benthic_year_long, benthic_sig_table, variable_name = "fma_mean", ylab = "Fleshy macroalgal cover") +
  scale_y_continuous(labels = label_percent(accuracy = 1)) +
  theme(legend.position = "none")

p_fma_height <- make_lp_summary(benthic_year_long, benthic_sig_table, variable_name = "fma_height_mean", ylab = "Fleshy macroalgae height (cm)") +
  theme(legend.position = "none")

p_lc <- make_lp_summary(benthic_year_long, benthic_sig_table, variable_name = "lc_mean", ylab = "Live coral cover") +
  scale_y_continuous(labels = label_percent(accuracy = 1))

benthic_change <- p_fma / p_fma_height / p_lc + plot_layout(heights = c(10, 10, 10))
ggsave(here(fig_dir, "benthic_change.png"), plot = benthic_change, width = 7, height = 12)
```
...
benthic_cover_site_chg <- benthic_cover_site %>%
  ...
  pivot_wider(
    ...
    values_from = c(lc_mean, cca_mean, ta_mean, tas_mean, fma_mean, cma_mean, ainv_mean, peys_mean),
    names_sep = "_"
  ) %>%
  mutate(
    ...
    d_peys = peys_mean_2025 - peys_mean_baseline
  ) %>%
  pivot_longer(
    cols = starts_with("d_"),
    names_to = "metric",
...
results <- benthic_cover_site_long %>%
  pivot_wider(names_from = year, values_from = value) %>%
  group_by(variable) %>%
  summarise(
    p_value = wilcox.test(`2019`, `2025`, paired = TRUE)$p.value,
    mean_2019 = mean(`2019`, na.rm = TRUE),
...
ggsave(here("agrra_monitoring", "figs", "fma_genera.png"), width = 6, height = 4)
```


## Algal canopy height

- 2017-2019: AlgaePointCoverScaledBySite.xlsx, sheet = data
  - FHavg: FMA height (site average)
- 2025: BenthicAlgalHeight.xlsx, sheet = scaled
  - tFMA-H: FMA height (site average)
...
### Bar plots
```{r}
ggplot(algal_height_year, aes(x = factor(year), y = mean)) +
  geom_col() +
  facet_wrap(. ~ site_cat) +
  geom_errorbar(
    aes(ymin = mean - sd, ymax = mean + sd),
...
# Prepare FMA delta table with bootstrap CIs
fma_delta <- algal_height_site %>%
  group_by(site, site_cat) %>%
  summarise(
    delta = fma_height[year == max(year)] - fma_height[year == min(year)],
    .groups = "drop"
  ) %>%
  group_by(site_cat) %>%
  summarise(
    mean_delta = mean(delta, na.rm = TRUE),
    ci = list(boot_mean_ci(delta)),
    wilcox_p = wilcox.test(delta, mu = 0)$p.value,
    .groups = "drop"
  ) %>%
  mutate(
    ci_lower = sapply(ci, `[[`, 1),
  ...
  select(site_cat, metric, mean_delta, ci_lower, ci_upper, wilcox_p) %>%
  mutate(
    mean_delta = round(mean_delta, 2),
    ci_lower = round(ci_lower, 2),
    ci_upper = round(ci_upper, 2),
...
p_delta_fmah <- ggplot(fma_delta,
  ...
  geom_point(size = 3) +
    scale_x_continuous(
    limits = c(-max(abs(fma_delta$ci_upper), abs(fma_delta$ci_lower)) * 1.1,
               max(abs(fma_delta$ci_upper), abs(fma_delta$ci_lower)) * 1.1)
  ) +
  facet_wrap(~ site_cat) +
  labs(
    x = "Mean change (cm)",
...
p_delta_pc / p_delta_fmah +
  plot_layout(heights = c(3, 1))

ggsave(here("agrra_monitoring", "figs", "benthic_delta_panel.png"), width = 6, height = 4)
```




## Coral spp. composition
...
# no longer have rel_mean by site
# ggplot(coral_fam_site %>%
#          filter(family %in% coral_fams),
...
coral_delta_table <- coral_fam_site %>%
  filter(site_cat == "NEMMA", year %in% c(2017, 2025), family %in% coral_fams) %>%
  select(year, site, family, avg) %>%
...
ggsave(here(fig_dir, "coral_fam_delta.png"), plot = p_delta_coral, width = 5, height = 6)
```
...
### Bar plots
```{r}
ggplot(recruits_fam_year %>% filter(year == 2025 & size == "total" & family != "All species"), aes(x = family, y = mean)) +
  geom_col() +
  facet_wrap(. ~ site_cat) +
  geom_errorbar(
    aes(ymin = mean, ymax = mean + sd),
...
### Dot plots by year
```{r}
ggplot(recruits_fam_year %>% filter(family_code == "ALL" & size != "total"),
...
## Coral spp. panel
```{r}
fam_levels <- rev(sort(unique(coral_fam_year$family)))

recruits_fam_2025 <- recruits_fam_year %>%
  ungroup() %>%
  filter(
    year == 2025,
    size == "total",
  ...
  mutate(
    ...
    metric = "Coral recruits"
  ) %>%
  select(site_cat, family, metric, mean, sd)

coral_fam_2025 <- coral_fam_year %>%
  filter(year == "2025") %>%
  mutate(
    family = factor(family, levels = fam_levels),
    metric = "Coral cover"
  ...
  select(site_cat, family, metric, mean, sd) %>%
  bind_rows(recruits_fam_2025)


make_family_plot <- function(df, site, metric_name, y_limits = NULL, y_label = NULL) {
  df_filtered <- df %>% 
    filter(site_cat == site, metric == metric_name)
  
  ggplot(df_filtered, aes(x = family, y = mean)) +
    geom_col() +
    geom_errorbar(aes(ymin = mean, ymax = mean + sd), width = 0.2) +
    coord_flip() +
    {if(!is.null(y_limits)) ylim(y_limits)} +   # optional y-axis limits
    labs(
      y = y_label,
      x = NULL,
      title = site
...
recruit_ylim <- c(
  0,
  max(coral_fam_2025 %>% 
        filter(metric == "Coral recruits") %>% 
        mutate(upper = mean + sd) %>% 
        pull(upper))
)


p1 <- make_family_plot(coral_fam_2025, "NEMMA", "Coral cover", y_limits = cover_ylim, y_label = "Mean Benthic Cover") +
  scale_y_continuous(labels = percent_format(accuracy = .1))
p2 <- make_family_plot(coral_fam_2025, "NDNP", "Coral cover", y_limits = cover_ylim) +
  scale_y_continuous(labels = percent_format(accuracy = .1))
p3 <- make_family_plot(coral_fam_2025, "NEMMA", "Coral recruits", y_limits = recruit_ylim, y_label = "Mean Recruit Density") +
  labs(title = NULL) +
  theme(axis.title.y = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank())
p4 <- make_family_plot(coral_fam_2025, "NDNP", "Coral recruits", y_limits = recruit_ylim) +
  labs(title = NULL) +
  theme(axis.title.y = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank())
...
ggsave(plot = p, here("agrra_monitoring", "figs", "coral_fam_comb.png"), width = 6, height = 5)
```
...
### Bar plots
```{r}
p_tot <- ggplot(fish_bm_year %>% filter(family == "Total"), aes(x = factor(year), y = mean)) +
  geom_col() +
  facet_wrap(. ~ site_cat) +
  geom_hline(yintercept = 1300*1000/100, linetype = "dashed", color = "black") + # Karr et al 2015 - 1300 kg/ha - converting to g/100m2
  # annotate("text", x = 3.5, y = 13500, size = 3, hjust = 1, label=c('Caribbean mean unfished biomass')) +
  geom_errorbar(
    aes(ymin = mean - sd, ymax = mean + sd),
    width = 0.2      # width of the error bars
  ) +
  labs(x = NULL, y = "Total biomass (g/100m2)") +
  theme_bw()

p_herb <- ggplot(fish_bm_year %>% filter(family == "Herbivores"), aes(x = factor(year), y = mean)) +
  geom_col() +
  facet_wrap(. ~ site_cat) +
  geom_hline(yintercept = 29.2*100, linetype = "dashed", color = "black") + # Edwards et al 2014 - 29.2 g/m2 - converting to g/100m2
  # annotate("text", x = 3.5, y = 3100, size = 3, hjust = 1, label=c('Caribbean mean unfished biomass')) +
  geom_errorbar(
    aes(ymin = mean - sd, ymax = mean + sd),
    width = 0.2      # width of the error bars
  ) +
  labs(x = NULL, y = "Herbivore biomass (g/100m2)") +
  theme_bw()

p <- p_tot / p_herb
p

ggsave(plot = p, here("agrra_monitoring", "figs", "fish_bm_bar.png"), width = 6, height = 6)
```
...
#### By site category
```{r}
fish_bm_sig_table <- fish_bm_site %>%
  group_by(site_cat, family) %>%
  summarise(
    values_by_year = list(split(avg, year)),
    .groups = "drop"
  ) %>%
  mutate(
    # paired Wilcoxon test for nonparametric data
  ...
  select(site_cat, variable = family, p_value, sig_label)

fish_bm_year <- fish_bm_year %>%
...
p1 <- make_lp_summary(fish_bm_year, fish_bm_sig_table, variable_name = "Total", ylab = "Total fish biomass") +
  geom_hline(yintercept = 1300*1000/100, linetype = "dashed", color = "black") + # Karr et al 2015 - 1300 kg/ha - converting to g/100m2
  theme(legend.position = "none")

p2 <- make_lp_summary(fish_bm_year, fish_bm_sig_table, variable_name = "Herbivores", ylab = "Herbivore biomass") +
  geom_hline(yintercept = 29.2*100, linetype = "dashed", color = "black") # Edwards et al 2014 - 29.2 g/m2 - converting to g/100m2

fish_bm_change <- p1 / p2 + plot_layout(heights = c(10,10))
fish_bm_change
ggsave(here(fig_dir, "fish_bm_change.png"), plot = fish_bm_change, width = 6, height = 8)
```
...
scar_length_site <- scar_length_long %>%
  ...
  mutate(length_mid = if_else(is.na(length_mid), 40, length_mid)) # considering >40 to be 40 bc scarids are rarely larger

scar_length_year_prop <- scar_length_site %>%
...
scar_length_year <- scar_length_site %>%
  group_by(site, site_cat, year) %>%
  summarise(
    mean_size = weighted.mean(length_mid, mean_prop, na.rm = TRUE),
    .groups = "drop"
  )
...
#### Delta plots
```{r}
scar_length_year_chg <- scar_length_year %>%
  mutate(
    period = if_else(year %in% c(2017, 2019), "baseline", "2025")
  ) %>%
  pivot_wider(
    id_cols = c(site, site_cat),
    names_from = period,
  ...
  mutate(delta_size = `2025` - baseline)

scar_length_delta <- scar_length_year_chg %>%
  group_by(site_cat) %>%
  summarise(
    mean_delta = mean(delta_size, na.rm = TRUE),
    ci = list(boot_mean_ci(delta_size)),
    wilcox_p = wilcox.test(delta_size, mu = 0)$p.value,
    .groups = "drop"
  ) %>%
  mutate(
    ci_lower = sapply(ci, `[[`, 1),
  ...
  select(-ci) %>%
  mutate(metric = "Scarid length")

ggplot(scar_length_delta,
       aes(x = mean_delta, y = metric)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  geom_errorbarh(aes(xmin = ci_lower, xmax = ci_upper), height = 0.2) +
  geom_point(size = 3) +
  ...
  scale_x_continuous(
    limits = c(-max(abs(scar_length_delta$ci_upper), abs(scar_length_delta$ci_lower)) * 1.1,
               max(abs(scar_length_delta$ci_upper), abs(scar_length_delta$ci_lower)) * 1.1)
  ) +
  theme_bw() +
  theme(
    strip.background = element_blank(),
...
ggsave(here("agrra_monitoring", "figs", "scar_length_delta.png"), width = 6, height = 2)
...
mosaicplot(t(mat), 
           ...
           main = paste("Scarid size distribution by year —", "NEMMA"),
           xlab = "",
           ylab = "Size Bin",
...
mosaicplot(t(mat), 
           color = TRUE,           # color each year
           main = paste("Scarid size distribution by year —", "NDNP"),
           xlab = "",
           ylab = "Size Bin",
           las = 1)                # rotate y-axis labels
...
Data summary: (must use raw data because phase is not in any summaries)
- 2017-2019: phase data is within count data
- 2025: phase data is within taxonomic ID
- need to check that phases are present for all scaridae
...
fish_raw_hist <- read_excel(here("agrra_monitoring", "data_raw", "ATG_2017-2022", "Raw", "MSAccess-FishRaw.xlsx"), sheet = "Counts Raw") %>%
  ...
  mutate(phase = case_when(terminal_phase == TRUE ~ "Terminal",
                           TRUE                  ~ "Initial")) %>%
  select(year, site, site_cat, trans = transect, size_class, length_mid, common_name, common_family, family, species, phase, count)
...
chk_n_trans <- fish_raw %>%
  group_by(site, site_cat, year) %>%
  summarise(
    n_transects = n_distinct(trans),
    .groups = "drop"
  )
...
ggsave(here("agrra_monitoring", "figs", "scarid_phases.png"), width = 8, height = 6)
```
...
## Fish size compositions
```{r}
herb_spp_dens <- fish_raw %>%
  mutate(species = if_else(species == "NA NA" & family == "Scaridae", "Scaridae spp.",
                           if_else(species == "Kyphosus NA", "Kyphosus spp.", species)),
         year = ifelse(year %in% c(2017, 2019), "2017-2019", as.character(year))) %>%
  filter(family %in% c("Scaridae", "Acanthuridae", "Kyphosidae") & !is.na(species)) %>%
  group_by(year, site, site_cat, trans, family, species, size_class) %>%
  summarise(n = sum(count, na.rm = TRUE), .groups = "drop") %>%
  complete(year, site, site_cat, trans, family, species, size_class, fill = list(n = 0)) %>%
  mutate(density = n / 60) %>%
  group_by(year, site, site_cat, family, species, size_class) %>%
  summarise(
    mean_density = mean(density, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  group_by(year, site_cat, family, species, size_class) %>%
  summarise(
    mean_density = mean(mean_density, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(size_class = factor(size_class, levels = c("0_5", "6_10", "11_20", "21_30", "31_40")))

herb_fam_dens <- fish_raw %>%
  mutate(year = ifelse(year %in% c(2017, 2019), "2017-2019", as.character(year))) %>%
  filter(family %in% c("Scaridae", "Acanthuridae", "Kyphosidae") & !is.na(species)) %>%
  group_by(year, site, site_cat, trans, family, size_class) %>%
  summarise(n = sum(count, na.rm = TRUE), .groups = "drop") %>%
  complete(year, site, site_cat, trans, family, size_class, fill = list(n = 0)) %>%
  mutate(density = n / 60) %>%
  group_by(year, site, site_cat, family, size_class) %>%
  summarise(
    mean_density = mean(density, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  group_by(year, site_cat, family, size_class) %>%
  summarise(
    mean = mean(mean_density, na.rm = TRUE),
  ...
  mutate(size_class = fct_recode(size_class,
                                 ...
                                 "21-30" = "21_30",
                                 "31-40" = "31_40") %>% 
                           fct_relevel("0-5", "6-10", "11-20", "21-30", "31-40"))

ggplot(herb_fam_dens, aes(x = as.numeric(size_class), y = mean,
                          group = year, fill = year)) +
  geom_area(alpha = 0.4, position = "identity") +
  geom_line(aes(color = year), size = 1.2) +  # optional outline
  geom_point(aes(color = year), size = 2) +
  facet_grid(site_cat~family, scales = "fixed") +
  scale_x_continuous(
    breaks = 1:5,
    labels = levels(herb_fam_dens$size_class)
  ) +
  labs(
    x = "Size Class (cm)",
    y = "Mean Density (individuals / m²)",
    fill = "Year",
    color = "Year"
  ) +
  theme_pub()

herb_sizes <- ggplot(herb_fam_dens %>% filter(family != "Kyphosidae"), 
       aes(x = size_class, y = mean*10000, color = year)) +
  geom_point(position = position_dodge(width = 0.6), size = 2) +
  geom_errorbar(aes(ymin = (mean- sd)*10000, ymax = (mean + sd)*10000),
                width = 0.2,
                position = position_dodge(width = 0.6)) +
  facet_grid(site_cat ~ family, scales = "fixed") +
  scale_color_manual(values = c("cadetblue", "gray20")) +
  labs(
    x = "Size Class (cm)",
    y = "Mean Density (individuals / ha)",
    color = "Year"
  ) +
  theme_pub() +
  theme(legend.position = "bottom")
...
ggsave(here(fig_dir, "herb_sizes.png"), plot = herb_sizes, width = 10, height = 6)
```
...
benth_fish_site_chg <- benth_fish_site %>%
  filter(year %in% c(2017, 2019, 2025)) %>%
  mutate(year = paste0("y", year)) %>%   # make names safe for pivot
  select(site, year, fma_mean, herb_mean, scar_mean) %>%
  tidyr::pivot_wider(
    names_from = year,
    values_from = c(fma_mean, herb_mean, scar_mean)
  ) %>%
  mutate(
    fma_baseline = coalesce(fma_mean_y2017, fma_mean_y2019),
    fma_2025 = fma_mean_y2025,
    fma_change   = fma_mean_y2025 - fma_baseline,
    fma_pct_change = (fma_change / fma_baseline) * 100,
    herb_2025    = herb_mean_y2025,
    scar_2025 = scar_mean_y2025
  ) %>%
  select(site, fma_baseline, fma_2025, fma_change, fma_pct_change, herb_2025, scar_2025)
```
...
mod <- lm(fma_change ~ herb_2025, data = benth_fish_site_chg)
summary(mod)
r2 <- summary(mod)$r.squared
pval <- summary(mod)$coefficients[2,4]

ggplot(benth_fish_site_chg, aes(x = herb_2025, y = fma_change)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "steelblue") +
  labs(x = "Herbivore biomass", y = "Change in FMA cover") +
  annotate("text", x = Inf, y = Inf,
           label = paste0("R² = ", round(r2, 2), 
                          "\nP = ", signif(pval, 2)),
           hjust = 1.1, vjust = 1.2, size = 4) +
