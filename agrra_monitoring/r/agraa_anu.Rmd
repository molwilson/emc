...
coral_fam_year <- coral_spp_site %>%
  # 1️⃣ sum species within families per site × year × site_cat
  group_by(year, site_cat, site, family) %>%
  summarize(
    value = sum(avg, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  
  # 2️⃣ complete missing families per site
  group_by(year, site_cat, site) %>%
  complete(
  ...
  # 3️⃣ calculate mean and SD per family × year × site_cat (including zeros)
  group_by(year, site_cat, family) %>%
  summarize(
  ...
  # 4️⃣ calculate relative mean per year × site_cat
  group_by(year, site_cat) %>%
  mutate(
    rel_mean = ifelse(sum(mean) > 0, mean / sum(mean), 0)
  ) %>%
  ungroup()
