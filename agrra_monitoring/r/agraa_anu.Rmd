...
benthic_cover_site_chg <- benthic_cover_site %>%
  ...
  pivot_wider(
    ...
    values_from = c(lc_mean, cca_mean, ta_mean, tas_mean, fma_mean, cma_mean),
    names_sep = "_"
  ) %>%
  mutate(
...
p_delta <- p_fma / p_lc  # patchwork uses / for vertical layout
ggsave(here("agrra_monitoring", "figs", "lc_fma_delta.png"), plot = p_delta, width = 6, height = 8)
