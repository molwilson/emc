...
# ---- prepare and compute change (fixed approach) ----
df_change <- fish_bm_site %>%
  filter(year %in% c(2017, 2019, 2025)) %>%
  # pivot so we have avg_2017, avg_2019, avg_2025, and std_*
  pivot_wider(
    id_cols = c(site, family, site_cat),
    names_from = year,
    values_from = c(avg, std),
    names_glue = "{.value}_{year}"
  ) %>%
  # create baseline as earliest of 2017 or 2019 (coalesce picks first non-NA)
  mutate(
    baseline_avg = coalesce(avg_2017, avg_2019),
    baseline_std = coalesce(std_2017, std_2019)
  ) %>%
  # drop groups that lack either baseline or 2025
  filter(!is.na(baseline_avg), !is.na(avg_2025)) %>%
  # compute change and CIs (simple SE-based; adjust if you want bootstrap or paired)
  mutate(
    change = avg_2025 - baseline_avg,
    # If std columns are SD of site means, proceed; otherwise adapt
    sd_base = baseline_std,
    sd_2025 = std_2025,
    # handle possible NA std by treating as 0 (or better: compute from raw data)
    sd_base = if_else(is.na(sd_base), 0, sd_base),
    sd_2025 = if_else(is.na(sd_2025), 0, sd_2025),
    se_change = sqrt(sd_base^2 + sd_2025^2),
    ci_low = change - 1.96 * se_change,
    ci_high = change + 1.96 * se_change
  )
...
# --- 2) Forest plot (paneled by site_cat) --------------------------

p <- df_change %>%
  ggplot(aes(x = change, y = fct_reorder(family, change))) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey40") +
  geom_pointrange(aes(xmin = ci_low, xmax = ci_high), size = 0.6) +
  facet_wrap(~ site_cat, scales = "free_y") +
  labs(
    x = "Change in biomass (avg 2025 − avg baseline)",
    y = "Family",
    title = "Change in fish biomass from baseline (2017/2019) to 2025",
    subtitle = "Forest plot by family, faceted by site_cat"
  ) +
