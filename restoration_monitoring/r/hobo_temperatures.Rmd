...
temp_data <- rbind(cades, tpb, jumby) %>%
  ...
  # accounting for data gaps - adding NA so they don't plot connecting lines
  arrange(logger, date_time) %>%
  group_by(logger) %>%
  mutate(gap = difftime(date_time, lag(date_time), units = "hours"),
         temp_c = if_else(gap > 6, NA_real_, temp_c)  # adjust threshold!
  ) %>%
  ungroup()
...
ggsave(here(fig_folder, "hobo_summary.png"), width = 8, height = 4)
...
ggplot(temp_data,
  ...
  theme_bw()
ggsave(here(fig_folder, "hobo_nursery.png"), width = 6, height = 4)
...
#   labs(x = "", y = "Temperature (°C)") +
#   theme_bw()
# ggsave(here(fig_folder, "hobo_cades.png"), width = 6, height = 4)
```
