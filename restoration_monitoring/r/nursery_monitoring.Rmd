...
status_nursery_post_bleaching_2024 <- nursery_mon %>%
  filter(date_floor == post_bleaching_2024 & 
           !(nursery == "York" & location %in% c("C1", "D1", "E1", "F1")) & # removing GBs because those would be double counted with deep nursery after moving it 
           !(nursery == "Cades" & location %in% c("A1", "B1"))) %>% # remove A1 and B1 from Cades as new ropes were added
  ...
  left_join(status_nursery_pre_bleaching_2024 %>%
              select(nursery, n_tot_baseline, n_adj)) %>%
  mutate(n_dead_rel_baseline = n_tot_baseline - n_tot_alive + n_adj) %>%
  select(nursery, n_tot_baseline, n_adj, n_tot_alive, n_healthy, n_diseased, n_pale, n_bleached, n_dead_rel_baseline) %>%
  adorn_totals("row") %>%
  ...
  select(nursery, n_tot_baseline, n_adj, n_tot_alive, n_healthy, n_diseased, n_pale, n_bleached, n_dead_rel_baseline, p_healthy, p_diseased, p_pale, p_bleached, p_healthy_rel_baseline, p_diseased_rel_baseline, p_pale_rel_baseline, p_bleached_rel_baseline, p_dead_rel_baseline, p_survival_rel_baseline)
...
```
### Comparing only genotypes present in all 3 nurseries
...
status_genotype_nursery <- nursery_mon %>%
  ...
  filter(! genotype %in% c("APAL58", "APAL53", "APAL60", "APAL46")) # removing temporarily until data is fixed
