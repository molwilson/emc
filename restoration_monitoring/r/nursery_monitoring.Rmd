...
- Compare Cades/DN/York performance
...
frags_lab <- read_excel(here("restoration_monitoring", "data_raw", "Nursery map.xlsx"), sheet = "Lab") %>% 
  clean_names() %>%
  select(genotype, n_lab = n_frags_initial, nursery = source)
...
status_nursery_pre_bleaching_2024 <- nursery_mon %>%
  ...
  left_join(frags_lab %>%
              group_by(nursery) %>%
              summarize(n_lab = sum(n_lab)) %>%
              rename(nursery = nursery)) %>%
...
status_nursery_post_bleaching_2024 <- nursery_mon %>%
  ...
  left_join(status_nursery_pre_bleaching_2024 %>%
              select(nursery, n_tot_baseline, n_lab)) %>%
  mutate(n_dead_rel_baseline = n_tot_baseline - n_tot_alive - n_lab) %>%
  select(nursery, n_tot_baseline, n_lab, n_tot_alive, n_healthy, n_diseased, n_pale, n_bleached, n_dead_rel_baseline) %>%
  adorn_totals("row") %>%
  ...
  select(nursery, n_tot_baseline, n_lab, n_tot_alive, n_healthy, n_diseased, n_pale, n_bleached, n_dead_rel_baseline, p_healthy, p_diseased, p_pale, p_bleached, p_healthy_rel_baseline, p_diseased_rel_baseline, p_pale_rel_baseline, p_bleached_rel_baseline, p_dead_rel_baseline, p_survival_rel_baseline)
...
```{r}
# identify genotypes present at all 3 comparison nurseries
genotype_comparisons <- nursery_mon %>%
  filter(date_floor == pre_bleaching_baseline_2024 &
           nursery %in% c("Cades", "DN", "York")
         ) %>%
...
status_genotype_nursery_baseline <- nursery_mon %>%
  filter(date_floor == pre_bleaching_baseline_2024 & 
  ...
  left_join(frags_lab, by = c("genotype", "nursery")) %>%
  mutate(n_lab = replace_na(n_lab, 0),
...
status_genotype_nursery <- nursery_mon %>%
  filter(date_floor == post_bleaching_2024 & 
           genotype %in% c(genotype_comparisons) &
           nursery %in% c("Cades", "DN", "York")
         ) %>%
  select(genotype, nursery, n_healthy, n_diseased, n_pale, n_bleached, n_other) %>%
  group_by(genotype, nursery) %>%
  summarize(n_healthy = sum(n_healthy),
            n_diseased = sum(n_diseased),
            n_pale = sum(n_pale),
  ...
  left_join(status_genotype_baseline %>%
              select(genotype, nursery, n_tot_baseline, n_lab)) %>%
  mutate(n_dead_rel_baseline = n_tot_baseline - n_tot_alive - n_lab) %>%
  select(genotype, nursery, n_tot_baseline, n_lab, n_tot_alive, n_healthy, n_diseased, n_pale, n_bleached, n_dead_rel_baseline) %>%
  distinct() %>%
  mutate(p_healthy = n_healthy/n_tot_alive*100, # relative to current total
         p_diseased = n_diseased/n_tot_alive*100, 
         p_pale = n_pale/n_tot_alive*100,
  ...
  select(genotype, nursery, n_tot_baseline, n_lab, n_tot_alive, n_healthy, n_diseased, n_pale, n_bleached, n_dead_rel_baseline, p_healthy, p_diseased, p_pale, p_bleached, p_healthy_rel_baseline, p_diseased_rel_baseline, p_pale_rel_baseline, p_bleached_rel_baseline, p_dead_rel_baseline, p_survival_rel_baseline)


### need to work on visualization/stats
ggplot(status_genotype_nursery %>%
         filter(genotype != "APAL58"), # removing temporarily until data is fixed
       aes(x = genotype, y = p_survival_rel_baseline, fill = nursery)) +
