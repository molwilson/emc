...
collection <- read_excel(here("restoration_monitoring", "data", "Genotype monitoring master.xlsx"), sheet = "collection") %>% 
  clean_names() %>%
  rename(collection_site = site)

nursery <- read.csv(here("nursery_monitoring", "data", "genotype locations.csv"))
...
genotype_tracking <- collection %>%
  select(genotype, collection_site) %>%
  left_join(nursery %>%
  ...
  mutate_at(c(5:10), ~replace_na(.,0)) %>%
  arrange(genotype)

write.csv(genotype_tracking, here("restoration_monitoring", "data", "genotype tracking.csv"), row.names=FALSE)
```
