...
# fig directory
fig_dir <- here("restoration_monitoring", "figs", "25-26_Q3")
```
...
- Rates of infection, bleaching, etc. are currently calculated as the mean of % incidence at each monitoring date across the quarter. Eventually I need to incorporate longer-term performance into genotype tracking sheet.
- Survivorship (number of frags at end vs. beginning + additions) currently includes diseased frags in total frag number at both beginning and end. Does this make sense?

```{r}
# import genotype tracking - genebank data

gb_mon <- read_excel(here("restoration_monitoring", "data_raw", "Genotype tracking.xlsx"), sheet = "archive - genebank monitoring") %>% 
  clean_names() %>%
...
nurs_mon_last <- nursery_mon %>%
  ...
  filter(!(nursery == "Cades" & location == "A4")) %>% # Cades A4 was outplanted since last monitoring
     mutate(location = if_else(
    nursery == "York" & location == "D1",
    "F5",
    location)) %>% # York D1 moved to F5 since last monitoring
  mutate(location = if_else(
    nursery == "York" & location == "G1",
    "D1",
    location)) # York G1 moved to D1 since last monitoring
...
frags_nursery <- nurs_gb_mon_last %>%
  ...
  select(nursery, n_frags = n_tot, p_healthy, p_heatstress, p_diseased, n_spp, n_genotypes)
...
- Completely empty structures are currently omitted in map code - adjust this to include here?
```{r}
...
map_york <- read_excel(here("restoration_monitoring", "data_raw", "Nursery map.xlsx"), sheet = "York", 
                   range = "A1:G9", col_names = FALSE, .name_repair = letter_names) %>%
...
map_tpb <- read_excel(here("restoration_monitoring", "data_raw", "Nursery map.xlsx"), sheet = "TPB", 
                   range = "A1:C13", col_names = FALSE, .name_repair = letter_names) %>%
  pivot_longer(cols = everything(),
               names_to = NULL,
               values_to = "genotype",
               values_drop_na = TRUE) %>%
  mutate(
    location = str_extract(genotype, "^[A-Z]\\d+"),
    genotype = str_remove(genotype, "^[A-Z]\\d+\\s*")
  ) %>%
  # separate_rows(genotype, sep = "\\s*/\\s*")
  filter(genotype != "") %>% # remove empty structures
  bind_rows(read_excel(here("restoration_monitoring", "data_raw", "Nursery map.xlsx"), sheet = "TPB", 
                       range = "A16:H23", col_names = FALSE) %>% 
              pivot_longer(
                cols = everything(),
                names_to = NULL,
                values_to = "genotype",
...
map_cades_sculptures <- read_excel(here("restoration_monitoring", "data_raw", "Nursery map.xlsx"), sheet = "Cades Sculptures") %>%
  clean_names() %>%
  select(genotype, location) %>%
  mutate(nursery = "Cades Sculptures")
...
nursery_maps <- rbind(map_york, map_tpb, map_cades, map_cades_sculptures, map_jumby) %>%
  mutate(genotype = strsplit(as.character(genotype), " / ")) %>% 
  unnest(genotype) %>%
  filter(!str_detect(genotype, "GB")) %>% # removing genebanks
...
nursery_locations <- nurs_mon_last %>%
              ...
              rename(york_locations = "York",
                     ...
                     cades_sculptures = "Cades Sculptures",
...
diff_nmon <- anti_join(nurs_mon_last %>% select(nursery, location, genotype), 
                       ...
                       by = c("nursery", "location", "genotype")) %>%
  anti_join( # removing GB locations because those are not in maps by genotype
   rbind(map_york, map_tpb, map_cades, map_cades_sculptures, map_jumby) %>%
     filter(str_detect(genotype, "GB")) %>% select(nursery, location),
    by = c("nursery", "location")
  )
...
collection <- read_excel(here("restoration_monitoring", "data_raw", "Genotype tracking.xlsx"), sheet = "collection") %>%
  ...
  mutate(across(everything(), 
         ...
         collection_date = as.Date(as.numeric(date), origin = "1899-12-30"),
...
outplants <- read_excel(here("restoration_monitoring", "data_raw", "Genotype tracking.xlsx"), sheet = "outplants") %>% 
  ...
  filter(!is.na(date)) %>%
  select(date, site, species, genotype, id, n_frags)
```
...
outplant_monitoring <- read_excel(here("restoration_monitoring", "data_raw", "genotype tracking.xlsx"), sheet = "outplant monitoring") %>% 
  clean_names() %>%
  filter(!is.na(n_healthy) & # filtering out any genotypes with missing data which includes "not found"
          date_monitored >= as.Date(b_start) & 
          date_monitored <= as.Date(b_end)) %>% 
  group_by(id, site) %>%
  slice_max(date_monitored, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  mutate(genotype = substr(id, 1, 6),
         species = substr(id, 1, 4),
         n_disease = replace_na(n_disease, 0),
         n_planted = coalesce(as.numeric(n_planted), n_healthy + n_disease),
...
datatable(outplant_perf_genotype,
          rownames = FALSE) %>%
  formatRound("outplant_survival", 3)
```
...
genotype_tracking <- collection %>%
  ...
  arrange(genotype) %>%
  select(updated, genotype, active, productive, collection_date, collection_site, collection_lat, collection_lon, york_locations, tpb_locations, cades_locations, jumby_locations, gb_locations, n_frags, n_outplanted, n_outplants, outplant_survival)
...
ggsave(here(fig_dir, "hobo_summary.png"), width = 6, height = 4)
...
ggplot(temp_data,
  ...
  theme_bw()
ggsave(here(fig_dir, "hobo_nursery.png"), width = 6, height = 4)
```