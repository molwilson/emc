...
# fig directory
fig_dir <- here("restoration_monitoring", "figs", "25-26_Q4")
```
...
nurs_mon_last <- nursery_mon %>%
  ...
  ungroup() %>%
  select(date, nursery, location, species, genotype, starts_with("n_"))
```

### By nursery

```{r}
frags_nursery <- nurs_mon_last %>%
  group_by(nursery) %>%
...
nursery_maps <- rbind(map_york, map_tpb, map_cades, map_cades_sculptures, map_jumby) %>%
  ...
  unnest(genotype) %>%
  filter(!str_detect(genotype, "GB")) %>% # removing genebanks for now
