...
Current nursery fragment counts
```{r}
...
genotype_tracking <- collection %>%
  ...
  mutate(productive = case_when((york_locations == "") & (tpb_locations == "") & (cades_locations == "") ~ "N",
         .default = "Y")) %>%
  select(genotype, active, productive, collection_date, collection_site, collection_lat, collection_lon, york_locations, tpb_locations, cades_locations, gb_locations, lab, mean_infection, survivorship_bl_2023 = survivorship, min_thermtol_bl_2023 = min_thermtolerance) # removing outplant results for now until we start outplanting again
