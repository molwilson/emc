...
rf_benthic <- read_excel(here("restoration_monitoring", "data_raw", "ecological monitoring.xlsx"), sheet = "benthic") %>% 
  clean_names() %>%
  # which transects from OG TPB do we want to include? currently includes all, but could be 1-2
...
ggplot(rf_pc_cat_site, 
  ...
  facet_wrap(vars(monitoring, site), nrow = 2) +
  theme_void() +
...
```{r}
# looking at changes in hard corals
ggplot(rf_pc_cat_site, aes(x = monitoring, y = pc_mean, group = site, fill = site)) +
  geom_col() +
...
```{r, eval = F}
# Leftover code from comparing restoration and non-restoration sites
...
ggplot(diadema_site 
       # %>%
       #   fct_relevel(monitoring, levels = c("basline", "6month"))
       , 
       aes(x = monitoring, y = density_mean)) +
...
rf_fish <- read_excel(here("mma", "data", "mma_coral_fish.xlsx"), sheet = "fish") %>% 
  clean_names() %>%
  rbind(read_excel(here("restoration_monitoring", "data", "ecological_monitoring.xlsx"), sheet = "fish") %>%
          clean_names() %>%
