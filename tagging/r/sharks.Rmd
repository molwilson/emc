...
shark_data <- left_join(temps, depth, by = c("pttid", "date")) %>%
  ...
  left_join(pings %>% select(pttid, date, lat, lon), by = c("pttid", "date")) %>%
  left_join(tagging %>%
              select(pttid, sex, species, length_tot, length_sex), 
            by = c("pttid"))
```
...
```{r}
# load logos
logo_folder <- here("tagging", "logos")
logo_anublue <- rasterGrob(readPNG(here(logo_folder, "AnuBlue.png")), interpolate = TRUE)
logo_gef <- rasterGrob(readJPEG(here(logo_folder, "GEF.jpg")), interpolate = TRUE)
logo_sgp <- rasterGrob(readJPEG(here(logo_folder, "SGP.jpg")), interpolate = TRUE)
logo_sgp_small <- editGrob(logo_sgp, vp = viewport(width = 0.6, height = 0.6))
logo_undp <- rasterGrob(readPNG(here(logo_folder, "UNDP.png")), interpolate = TRUE)

# combine logos into strip
logo_strip <- plot_grid(
  logo_anublue, logo_gef, logo_sgp_small, logo_undp,
  nrow = 1,
  rel_widths = c(1, 1, 1, 1)  # tweak if one logo needs more space
)
...
p_base_logos <- plot_grid(
  p_base,
  logo_strip,
  ncol = 1,
  rel_heights = c(.5, 0.15)  # logo height ~12% of total
)
```
...
p_depth_indv <- ggplot(shark_data, aes(x = date)) +
  ...
  theme(legend.position = "top") +
  labs(color = "", y = "Depth (m)", x = "")
ggsave(here("tagging", "figs", "shark_depth_indv.png"), plot = p_depth_indv, width = 8, height = 6)
