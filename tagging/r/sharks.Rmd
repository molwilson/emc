...
# Import + format raw ARGOS data

- each shark has inividual sheet
...
file_path <- here("tagging", "data_raw", "DataRw.xlsx")
...
process_sheet_split <- function(sheet_name) {
  ...
  extract_block <- function(i) {
    ...
    data <- data %>% mutate(pttid = sheet_name)
  ...
  # Split into a named list by type
  split_blocks <- blocks %>% 
...
# animated plots - indv. sharks
animate_indv <- function(pttid, shark_df, atg, out_dir = "mapping/figs") {
  
  p_anim <- ggplot() +
    geom_sf(data = atg, fill = "lightblue", color = "lightblue") +
    geom_path(
      data = shark_df %>% filter(pttid == !!pttid),
      aes(x = x, y = y),
  ...
  # save
  gif_path <- here::here("tagging", "figs", paste0("shark_track_", pttid, ".gif"))
